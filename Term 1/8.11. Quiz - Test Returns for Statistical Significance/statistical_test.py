...
def analyze_returns(net_returns):
    ...
    n = net_returns.shape[0]

    x_bar = net_returns.mean()  # sample mean
    se = net_returns.std() / np.sqrt(n)  # standard error
    t_value = x_bar / se  # statistic
    _, p_value = stats.ttest_1samp(
        net_returns, null_hypothesis)  # t_value, p_value
    # t_value, p_value = stats.ttest_1samp(net_returns, null_hypothesis)  # t_value, p_value

    return t_value, p_value / 2.0
